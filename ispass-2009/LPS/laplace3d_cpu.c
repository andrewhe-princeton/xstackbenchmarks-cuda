...
void _Z13GPU_laplace3diiiiPfS__OC_1(uint32_t, uint32_t, uint32_t, uint32_t, float*, float*, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t) __ATTRIBUTELIST__((noinline, nothrow));
...
_Z13GPU_laplace3diiiiPfS__OC_1(NX, NY, NZ, pitch, ((float*)__FIXME__call35), d_u2, bx, by, 1, 32, 4, 1, j, k, 0, l, m, 0);
}
}
}
}
...
//INSERT COMMENT IFELSE: entry
  ...
  __FIXME__4 = 
//__FIXME__INTRINSIC_CALL__
llvm_OC_nvvm_OC_mul24_OC_i(j + 1, 34);
  ind_h = i + 1 + __FIXME__4 + 204;
  __FIXME__5 = 
//__FIXME__INTRINSIC_CALL__
llvm_OC_nvvm_OC_mul24_OC_i(x, 32);
  i = i + __FIXME__5;
  __FIXME__6 = 
//__FIXME__INTRINSIC_CALL__
llvm_OC_nvvm_OC_mul24_OC_i(x, 4);
  __FIXME__add25 = j + __FIXME__6;
  __FIXME__7 = 
//__FIXME__INTRINSIC_CALL__
llvm_OC_nvvm_OC_mul24_OC_i(__FIXME__add25, pitch);
  indg_h = i + __FIXME__7;
  if (i >= 0) {
  if (i < NX) {
  if (__FIXME__add25 >= 0) {
...
//INSERT COMMENT IFELSE: if.end34
  __FIXME__8 = 
//__FIXME__INTRINSIC_CALL__
llvm_OC_nvvm_OC_mul24_OC_i(__FIXME__threadIdx_2e_y + 1, 34);
...
llvm_OC_nvvm_OC_mul24_OC_i(x, 4);
  ...
  __FIXME__11 = 
//__FIXME__INTRINSIC_CALL__
llvm_OC_nvvm_OC_mul24_OC_i(__FIXME__add47, pitch);
...
for(int32_t k = 0; k < NZ;   k = k + 1){
  if (__FIXME__3 != 0) {
  uint32_t __FIXME__12 = 
//__FIXME__INTRINSIC_CALL__
llvm_OC_nvvm_OC_mul24_OC_i(NY, pitch);
