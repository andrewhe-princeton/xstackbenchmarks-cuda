...
int main(int argc, char **argv){
  ...
  if(  argc, (const char**)argv, "nz", &NZ ) { // Original: if( cutGetCmdLineArgumenti( argc, (const char**)argv, "nz", &NZ) ) {
  ...
  if(  argc, (const char**)argv, "repeat", &REPEAT ) { // Original:  if( cutGetCmdLineArgumenti( argc, (const char**)argv, "repeat", &REPEAT) ) {
  ...
  for (dev = 0; dev < deviceCount; ++dev) {                                
      ...
      cudaGetDeviceProperties(&deviceProp, dev);  // Original: CUDA_SAFE_CALL_NO_SYNC(cudaGetDeviceProperties(&deviceProp, dev));    
