...
int32_t atoi(uint8_t*);
void* malloc(uint64_t);
void free(void*);
int32_t fprintf(struct __FIXME__l_struct_struct_OC__IO_FILE*, uint8_t*, ...);
...
//INSERT COMMENT IFELSE: entry
  // CODE MOTION: VAR DECLARATION
  int32_t dump_code = atoi(argv[1]);
  // CODE MOTION: VAR DECLARATION
  int32_t n = atoi(argv[2]);
  // CODE MOTION: VAR DECLARATION
  int32_t tsteps = atoi(argv[3]);
  // CODE MOTION: VAR DECLARATION
  uint8_t* u = malloc(n * n * 8);
  // CODE MOTION: VAR DECLARATION
  uint8_t* v = malloc(n * n * 8);
  // CODE MOTION: VAR DECLARATION
  uint8_t* p = malloc(n * n * 8);
  // CODE MOTION: VAR DECLARATION
  uint8_t* q = malloc(n * n * 8);
  int32_t __FIXME__call38; // Unused
  int32_t __FIXME__call54; // Unused
...
void kernel(uint32_t tsteps, uint32_t n, double* u, double* v, double* p, double* q) {
  ...
  // CODE MOTION: VAR DECLARATION
  double mul1 = (((2 * DT) / DX) / DX);
  // CODE MOTION: VAR DECLARATION
  double mul2 = (((1 * DT) / DY) / DY);
  // CODE MOTION: VAR DECLARATION
  double __FIXME__div10 = (-(mul1) / 2);
  // CODE MOTION: VAR DECLARATION
  double b = (1 + mul1);
  // CODE MOTION: VAR DECLARATION
  double __FIXME__div12 = (-(mul2) / 2);
  // CODE MOTION: VAR DECLARATION
  double e = (1 + mul2);
...
// CODE MOTION: VAR DECLARATION
for(uint32_t j = 0; j < __FIXME__call;   j = j + 1){
// CODE MOTION: VAR DECLARATION
...
#pragma omp parallel for collapse(2)
for(uint32_t j = 0; j < __FIXME__call19;   j = j + 1){ // Re-declaration of j is fine here due to OpenMP scope rules
for(uint32_t k = 0; k < 256;   k = k + 1){ // Re-declaration of k is fine here due to OpenMP scope rules
...
void print_array(uint32_t n, double* u) {
  int32_t __FIXME__call11; // Unused
...
//INSERT COMMENT IFELSE: entry
  // CODE MOTION: VAR DECLARATION
  int32_t i = __FIXME__blockDim_2e_x * __FIXME__blockIdx_2e_x + __FIXME__threadIdx_2e_x + 1;
  if (i < (n - 1)) {
  v[(0 * n + i)] = 1;
  p[(i * n + 0)] = 0;
  q[(i * n + 0)] = v[(0 * n + i)];
...
for(int64_t j = 1; j < (n - 1);   j = j + 1){
  ...
  v[((n - 1) * n + i)] = 1;
for(int64_t j = n + -2; j >= 1;   j = j + -1){ // Re-declaration of j is fine here due to loop scope
